...
def get_all_stock_info():
    ...
    try:
        ...
        if not stock_data:
            download_result = crud_helper.update_stock_data(stock_symbol)
            if download_result:
                stock_data = crud_helper.get_all_stock_info(stock_symbol)
        ...
        if not stock_data:
            return render_template(
                'index.html',
                error=f"No data available for stock symbol: {stock_symbol}",
            )
        ...
        logging.error(f"Error fetching stock data for symbol {stock_symbol}: {e}")
        return render_template(
            'index.html',
